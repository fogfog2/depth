...
def draw_hole(img):
    ...
    #image smooth
    tt= cv2.resize(crop_gray, dsize =(20,20))
    crop_gray= cv2.resize(tt, dsize =(h,w),interpolation=cv2.INTER_LINEAR)

    #intergral image (평균계산 빠르게)
    isum= cv2.integral(crop_gray)
    ...
    for step in range(initial_step,1,-2):
